...
2. 因此，可先求 $L$：
    $$\begin{aligned}
    圓柱體積(V_{Cylinder}) &= 球體體積(V_{Sphere})\\\frac{\cancel{\pi} D_{Cylinder}^2L}{4} &= \frac{\cancel{\pi} D_{Sphere}^3}{6}\\\frac{\cancel{\pi} (0.2D_{Sphere})^2L}{4} &= \frac{\cancel{\pi} D_{Sphere}^3}{6}\\L &= \frac{50D_{Sphere}}{3}\end{aligned}$$
3. 可再求得面積比值（Ratio）：
...
---
### Problem 2.11
$$100\ \mathrm{kg_f} = \cfrac{(10\ \mathrm{m})^3\pi}{6}\cdot\left (\rho_g-\rho_{in}\right)g=\cfrac{(10\ \mathrm{m})^3\pi}{6}\cdot\left (1.2\ \mathrm{kg/m^3}-\cfrac{1\ \mathrm{atm}\cdot29\ \mathrm{g/mol}}{RT}\right)g$$
$$980.665\ \mathrm{N} = \cfrac{(10\ \mathrm{m})^3\pi}{6}\cdot\left (1.2\ \mathrm{kg/m^3}-\cfrac{1\ \mathrm{atm}\cdot29\ \mathrm{g/mol}}{RT}\right)g\rightarrow\underline{T=350.2541\ \mathrm{K}}　\blacksquare$$
...
5. 在 $h_{remove}$ 以上的顆粒來不及沉降去除，因此出流比例 $x_p=1-\cfrac{h_{remove}}{h}=1-\cfrac{0.0939\ \mathrm{m}}{1\ \mathrm{ft}}=\underline{0.6919}　\blacksquare$
---
### Problem 3.17
1. 粒徑很大，Re 必很大。
2. 求顆粒沉降速率 $V_{TS}=\cfrac{\eta}{\rho_gd_p}e^{(-3.07+0.9935J-0.0178J^2 \And J=ln(\frac{4\rho_g\rho_pd^3g}{8\eta^2}))}=1.6272\ \mathrm{\frac{m}{s}}$。
3. 確定在流出時間前會沉降的高度 $h_{remove}=24\ \mathrm{s}\cdot 1.6272\ \mathrm{\frac{m}{s}}=\underline{39.0537\ \mathrm{m}}　\blacksquare$
---
### Problem 3.22
1. 由題可知：$\left\{\begin{aligned}\frac{n_{1hr}}{n_0}=\frac{20\ \mathrm{\frac{mg}{m^3}}}{n_0}=\exp(\frac{-V_{TS}\cdot 1\ \mathrm{hr}}{14\ \mathrm{ft}})\\\frac{n_{2hr}}{n_0}=\frac{6\ \mathrm{\frac{mg}{m^3}}}{n_0}=\exp(\frac{-V_{TS}\cdot 2\ \mathrm{hr}}{14\ \mathrm{ft}}) \end{aligned}\right.$。解聯立方程式（上下相除），可得 $V_{TS}=0.0014\ \mathrm{\frac{m}{s}}$、$n_0=\underline{66.6667\ \mathrm{\mu m}}　\blacksquare$
2. 透過終端速度反求氣動粒徑：$V_{TS}=\cfrac{\rho_pd^2g}{18\eta}\rightarrow \underline{d=6.8856\cdot10^{-6}\ \mathrm{m}}　\blacksquare$
...
2. 沿垂直布朗運動（位移均值 = 沉降距離）
   $$V_{TS}=\cfrac{\rho_pd^2g}{18\eta}C_c=\cfrac{\rho_pd^2g}{18\eta}\cdot(1+\cfrac{2.52\lambda}{d_p})=1.4\cdot10^{-5}\ \mathrm{\frac{m}{s}}\\x=V_{TS}\cdot t=1.4\cdot10^{-5}\ \mathrm{\frac{m}{s}}\cdot5\ \mathrm{min}=\underline{4.151951396685\ \mathrm{mm}}　\blacksquare$$
3. 標準差
   $$\sigma=\sqrt{2Dt}=\sqrt{2\cfrac{kTC_c}{3\pi\eta d_p}t}=\sqrt{2\cfrac{kT\cdot(1+\cfrac{2.52\lambda}{d_p})}{3\pi\eta d_p}t}=\underline{0.17295107557531\ \mathrm{mm}}　\blacksquare$$
---
### Problem 7.4
1. 已知：
   - $C_c=2.93$
   ...
   - $\sigma=x_{rms}=\sqrt{2Dt}$
   - $D=\cfrac{kTC_c}{3\pi\eta d}$
2. 常態分佈：當目標為 32% 機率在 1 m 處發現時，表示以 x=0 為平均值，$\pm$ 1 m 之間的面積達 1-0.32=0.64。而由於為常態分佈，0.64 洽為 $\sigma=1$ 時之面積。
3. $\sigma=x_{rms}=\sqrt{2Dt}=\sqrt{2\cfrac{kTC_c}{3\pi\eta d}t}=\sqrt{2\cfrac{k(20\mathrm{\degree C})(2.93)}{3\pi(1.81\cdot10^{-5}\ \mathrm{Pa\cdot s})(0.1\ \mathrm{\mu m})}t}$
4. $\sigma=1\rightarrow x_{rms}=1\ \mathrm{m}$
5. $\sqrt{2\cfrac{k(20\mathrm{\degree C})(2.93)}{3\pi(1.81\cdot10^{-5}\ \mathrm{Pa\cdot s})(0.1\ \mathrm{\mu m})}t}=1\ \mathrm{m}\rightarrow \underline{t=719616616.91479\ \mathrm{s}=22.803761605035\ \mathrm{year}}　\blacksquare$
---
### Problem 7.5
1. 已知：
...
1. 已知：
   ...
   - $\mu=\cfrac{DL}{Q}$
   - $D=\cfrac{kTC_c}{3\pi\eta d}$
2. $C_c=1+\frac{\lambda}{d}(2.34+1.05e^{-0.39\frac{d}{\lambda}})=22.9764$
3. $\mu=\cfrac{DL}{Q}=\cfrac{\cfrac{kTC_c}{3\pi\eta d}L}{Q}=\cfrac{\cfrac{k(20\mathrm{\degree C})(22.9764)}{3\pi(1.81\cdot10^{-5}\ \mathrm{Pa\cdot s})(0.01\ \mathrm{\mu m})}1\ \mathrm{m}}{0.2\ \mathrm{\frac{L}{min}}}=0.0163$
4. 穿透率：$P=0.819e^{-11.5\mu}+0.975e^{-70.1\mu}=0.7096$
5. $去除率=100\%-穿透率=1-P=\underline{0.2903=29.034707049631\%}　\blacksquare$
---
### Problem 7.7
1. 已知：
...
2. 計算：
   $$\begin{aligned}
   N(t)&=2n_0\sqrt{\cfrac{Dt}{\pi}}\\
   \cfrac{8\cdot10^7}{A_{Sphere}}&=2n_0\sqrt{\cfrac{(6.95\cdot10^{-10}\ \mathrm{\frac{m^2}{s}})(5\ \mathrm{min})}{\pi}}\\
   \cfrac{8\cdot10^7}{D_{Sphere}^2\pi}&=2n_0\sqrt{\cfrac{(6.95\cdot10^{-10}\ \mathrm{\frac{m^2}{s}})(5\ \mathrm{min})}{\pi}}\\
   \cfrac{8\cdot10^7}{(32.5\ \mathrm{mm})^2\pi}&=2n_0\sqrt{\cfrac{(6.95\cdot10^{-10}\ \mathrm{\frac{m^2}{s}})(5\ \mathrm{min})}{\pi}}\\
   \underline{n_0}&=\underline{46,799,974,590,248\ \mathrm{m^{-3}}}　\blacksquare
   \end{aligned}$$
...
3. 由於 error 需介於 ±10%，故 $\cfrac{C}{C_0}=\left[\cfrac{1-10\%}{1},\ \cfrac{1+10\%}{1}\right]=\left[0.9,\ 1.1\right]$
4. 故：
   $$\begin{aligned}
   \cfrac{C}{C_0}&=\cfrac{U_0}{U}\\
   \left[0.9,\ 1.1\right]&=\cfrac{U_0}{(\frac{Q}{A})}\\
   \left[0.9,\ 1.1\right]&=\cfrac{(50\ \mathrm{\frac{mi}{hr}})}{\cfrac{(40\ \mathrm{\frac{L}{min}})}{\left(D_S^2\pi/4\right)}}\\
   \underline{D_s}&=\underline{\left[0.0058461841360979\ \mathrm{m},\ 0.0064631997449214\ \mathrm{m}\right]}　\blacksquare
   \end{aligned}$$
### Problem 10.2
#### Solution-1 簡單版
1. 已知：
...
2. $Q=10\ \mathrm{\frac{L}{min}}=0.6\ \mathrm{\frac{m^3}{hr}}$
3. $D_s=20\ \mathrm{mm}\geq4.05(Q)^{1/3}(d_a)^{2/3}=4.05(0.6)^{1/3}(d_a)^{2/3}\rightarrow\underline{d_a\leq14.1672916185\ \mathrm{\mu m}}　\blacksquare$
#### Solution-2 複雜版
1. 已知：
...
5. 由於 Stk = 1.6674 < 6，故使用式 (10.4) 評估誤差：
   ...
   3. 故，誤差為：$0.8930-1=\underline{−0.1070=−10.704406214522\%}　\blacksquare$
---
### Problem 10.4
...
2. 求 Stk 使用式 (10.4)：
   $$Stk=\frac{\tau U_0}{D_S}=\frac{(\frac{\rho_pd^2_p}{18\eta})(25\ \mathrm{\frac{m}{s}})}{1.3\ \mathrm{cm}}=\frac{(\frac{(1,000 \mathrm{\frac{kg}{m^3}})(20\ \mathrm{\mu m})^2}{18(1.81\cdot10^{-5}\ \mathrm{Pa\cdot s})})(25\ \mathrm{\frac{m}{s}})}{1.3\ \mathrm{cm}}=2.3611$$
3. 由於 Stk = 1.6674 < 6，故使用式 (10.7) 評估誤差：
   ...
   2. 故，誤差為：$1.4721-1=\underline{0.4721=47.20948429199\%}　\blacksquare$
---
### Problem 10.8
1. 已知：
   - $D_S=10\ \mathrm{mm}$
...
2. (a) 求直管損失：
   ...
   3. 求 $k_1,\ k_2$（==注意，直管不轉彎，故 $\theta=0\degree$ ==）：
      $$k_1=\cfrac{3LV_{TS}\cos(\theta)}{4D_SU}=\cfrac{3(0.3\ \mathrm{m})(0.0030\ \mathrm{\frac{m}{s}})\cos(0\degree)}{4(10\ \mathrm{mm})(1.2732\ \mathrm{\frac{m}{s}})}=0.0532\\k_2=\sqrt{1-k_1^{2/3}}=\sqrt{1-0.0532^{2/3}}=0.9266$$
   4. 求 Setting Loss：
...
2. 求最大數量濃度時必使用式 (12.12)，而當數量濃度最大時，初始濃度必最大。故：
   $$\begin{aligned}N(t)&=\cfrac{N_0}{1+N_0K_0t}\\
   \max N(t)&=\lim_{N_0\rightarrow\infin}\cfrac{N_0}{1+N_0K_0t}=\cfrac{1}{K_0t}\\&=\cfrac{1}{(3\cdot10^{-16}\ \mathrm{m^3/s})(5\ \mathrm{hr})}\\&=\underline{185185185185.19\ \mathrm{m^{-3}}}　\blacksquare\end{aligned}$$
### Problem 12.2
1. 已知：
...
2. 求數量濃度時必使用式 (12.12)：
   $$\begin{aligned}N(t)&=\cfrac{N_0}{1+N_0K_0t}\\
   10^{6}\mathrm{/cm^3}&=\cfrac{10^{7}\mathrm{/cm^3}}{1+(10^{7}\mathrm{/cm^3})(5\cdot10^{-16}\ \mathrm{m^3/s})t}\\ t&=\underline{1,800\ \mathrm{s}}　\blacksquare\end{aligned}$$
3. 求膠結粒徑時必使用式 (12.16)：
...
2. 解題策略：計算最終數量濃度，減少的數量即為膠結的顆粒數。故須先找出 $K_0$。
   $$K_0=\cfrac{4kTC_c}{3\eta}=\cfrac{4k(20\ \mathrm{\degree C})(1.1663)}{3(1.81\cdot10^{-5}\ \mathrm{Pa\cdot s})}=3.4774\cdot10^{-16}\ \mathrm{\frac{m^3}{s}}$$
3. 計算 N(t)：
...
2. 故根據式 (12.12)：
   $$\begin{aligned}N(t)&=\cfrac{N_0}{1+N_0K_0t}\\
   \frac{1}{13.5}\cdot10^6\ \mathrm{cm^{-3}}&=\cfrac{\frac{1}{2}\cdot10^6\ \mathrm{cm^{-3}}}{1+(\frac{1}{2}\cdot10^6\ \mathrm{cm^{-3}})K_0(200\ \mathrm{min})}\\\underline{K_0}&=\underline{9.5833333333333\cdot10^{-16}}\ \mathrm{\frac{m^3}{s}}　\blacksquare\end{aligned}$$
3. 又可反求粒徑：
...
1. 已知：
   ...
   - $t=2\ \mathrm{s}$
2. 求膠結粒徑時必使用式 (12.16)：
   $$d(t)=d_0\sqrt[3]{1+N_0K_0t}\rightarrow\frac{d(t)}{d_0}=\sqrt[3]{1+N_0K_0t}\\
   \frac{d(t)}{d_0}=\sqrt[3]{1+\left(\frac{3\cdot10^9}{\mathrm{cm^3}}\right)\left(7.0\cdot 10^{-10}\ \mathrm{\frac{cm^3}{s}}\right)(2\ \mathrm{s})}=\underline{1.7324782106818}　\blacksquare$$
---
### Problem 12.8
1. 已知：
...
3. (b)：
   1. 求題 (a) 的最終粒徑：
      $$\begin{aligned}d(t)&=d_0\sqrt[3]{1+N_0K_0t}\\&=d_{\overline{m}}\sqrt[3]{1+N_0K_0t}\\&=(CMD\cdot e^{1.5\ln^2(\sigma_g)})\sqrt[3]{1+N_0K_0t}\\
      &=((0.2\ \mathrm{\mu m})\cdot e^{1.5\ln^2(2.0)})\sqrt[3]{1+(3\cdot10^{14}/\mathrm{cm^3})(9.5301\cdot10^{-16}\ \mathrm{\frac{m^3}{s}})(200\ \mathrm{s})}\\&=1.1172\ \mathrm{\mu m}\end{aligned}$$
   2. 反求最終粒徑之 CMD：
   ...
   4. 取二個 $\overline{K}$ 均值：$\cfrac{9.5301\cdot10^{-16}\ \mathrm{\frac{m^3}{s}} + 5.9728\cdot10^{-16}\ \mathrm{\frac{m^3}{s}}}{2}=7.7515\cdot10^{-16}\ \mathrm{\frac{m^3}{s}}$
   5. 求最終質量濃度 $N(t)$：
...
- 粒徑變化率：$\cfrac{d(d_p)}{dt}=\cfrac{4D_vM}{R\rho_pd_p}\left(\cfrac{P_\infin}{T_\infin}-\cfrac{P_d}{T_d}\right)\Phi\ \And\Phi=\cfrac{2\lambda+d_p}{d_p+5.33\frac{\lambda^2}{d_p}+3.42\lambda}$
- 液滴溫度：
  $$T_d=\left\{\begin{aligned}&\cfrac{D_vMH}{Rk_v}\left(\cfrac{P_d}{T_d}-\cfrac{P_\infin}{T_\infin}\right)+T_\infin  &\\&\cfrac{(6.65+0.345T_\infin+0.0031T_\infin^2)(S_R-1)}{1+(0.082+0.00782T_\infin)S_R}+T_\infin\ @\ \text{Unit: }Q(\mathrm{\frac{m^3}{hr}})   &\mathrm{for\begin{aligned}S_R&\in[0,\ 5]\\ T&\in[273\ \mathrm{K},\ 313\ \mathrm{K}]\end{aligned}}\end{aligned}\right.$$
## Problem-C13
### Problem 13.1
1. 已知：
...
### Problem 13.4
1. 已知：
   - $T_{\infin}=20\ \mathrm{\degree C}$
   - $S_R=4.3$
2. 本題欲求膨脹比，必透過式 (13.4) 求得，此時需 $T_2$。絕熱膨脹時溫度變化式 (13.15) 求得：
   $$\begin{aligned}T_d&=\cfrac{(6.65+0.345T_\infin+0.0031T_\infin^2)(S_R-1)}{1+(0.082+0.00782T_\infin)S_R}\ \mathrm{\degree C}+T_\infin\\&=\cfrac{(6.65+0.345(20)+0.0031(20)^2)((4.3)-1)}{1+(0.082+0.00782(20))(4.3)}\ \mathrm{\degree C} + (20\ \mathrm{\degree C})\\&=44.1008\ \mathrm{\degree C}\end{aligned}$$
3. 故：
...
3. 故：
   $$\begin{aligned}RH=S_R=K_R&=e^{\cfrac{4\gamma M}{\rho RTd^*}}\\&=e^{\cfrac{4(75.9450\ \mathrm{\frac{dyne}{cm}}) (18\ \mathrm{\frac{g}{mol}})}{(1,000\ \mathrm{\frac{kg}{m^3}}R(274\ \mathrm{K})(0.2\ \mathrm{\mu m})}}\\&=\underline{1.0121 = 101.20732870555\%}　\blacksquare\end{aligned}$$
---
### Problem 13.6
1. 已知：
   1. $d_p=2\ \mathrm{\mu m}$
...
2. 由於 $S_R=0$ 故 $P_\infin=0$。使得式 (13.19)：$t=\cfrac{R\rho_pd_p^2}{8D_vM\left(\frac{P_d}{T_d}-\frac{P_\infty}{T_\infty} \right)}=\cfrac{R\rho_pd_p^2}{8D_vM\frac{P_d}{T_d}}$
...
2. 解 $\sigma_{Air}=\sigma_{Particle}$：
...
### Problem 16.7
#### Question
> Repeat [problem 16.6](#problem-166), but include the effect of air molecules. Assume that $\lambda$ = 0.5 μm.
#### Solution
1. 已知：
...
2. 因此：
$$\begin{aligned}
   L_v&=\cfrac{-\ln(0.02)}{\sigma_e}\\L_v&=\cfrac{-\ln(0.02)}{\cfrac{32\pi^3(m-1)^2\cdot1.054}{3\lambda^4n}}\\L_v&=\cfrac{-\ln(0.02)}{\cfrac{32\pi^3((1.00028))-1)^2\cdot1.054}{3(0.5\ \mathrm{\mu m})^4\frac{(1\ \mathrm{atm})N_A}{R(293\ \mathrm{K})}}}
   \\&=\underline{224,084.6518412\ \mathrm{m}}　\blacksquare\end{aligned}$$