...
# 3.2 質量平衡
- 基本結構  
  ![批次反應槽](/f0eee7e95999bd51ed1534d2fab6b5f0.svg) 
	- 其中：
		- $S^0$：初始基質濃度。
		- $Q$：供料流速。
		- $V$：槽體體積。
...
- 而除了活性菌體，亦有死亡之惰性菌體及溶解性微生物產物[註解](#a.4)：
	- $X_i$：出流水惰性菌體濃度。
  ...
  - $r_{ut}$：基質利用率。   
	...
	- $\rm SMP$ (Soluble Microbial Products)：微生物產生的溶解性物質濃度，由細胞裂解釋放，在合成時損失或因其他目的分離出來。包含基質代謝產生之 $\rm UAP$ (Utilization Associated Products) 及細胞合成衰敗產生之 $\rm BAP$ (Biomass Bssociated Products)。
		- $\rm SMP = UAP + BAP$
...
根據微生物生長速率($\mu$)：  
$$\mu=\frac{1}{X_a}\frac{dX_a}{dt}=\mu_{syn}+\mu_{dec}=\hat{\mu}\frac{S}{K+S}-b$$  
且生物生長率($\mu$)與基質利用率($q$)之關係：  
$$\hat{\mu}=Y\hat{q}$$  
定義：淨活體細胞生長率($r_{net}$)表單位時間之微生物生長量，故等於細胞比生長速率($\mu$)與活體細胞濃度($X_a$)之乘積($r_{net}=\mu X_a$)。  
得淨活體細胞生長率($r_{net}$)：  

$$\begin{align}r_{\text{net}} &= \mu X_a \\&= \hat{\mu}\frac{S}{K+S}X_a - bX_a \\&= \hat{q}YX_a\frac{S}{K+S} - bX_a\end{align}$$
...
$$\begin{aligned}\mu=\frac{r_{net}}{X_a}=Y\frac{\hat{q}S}{K+S}-b\\=Y\left (\frac{\hat{q}S}{K+S}-m\right )\end{aligned}$$

- 其中：$m$：最低維持能量($^{M_S}/_{M_XT}$)
...
已知：基質利用率 $r_{ut}=-X_a\frac{\hat{q}S}{K+S}$；淨微生物生長速率 $\mu=Y\frac{\hat{q}S}{K+S}-b$，帶入式 $(a)$、$(b)$。得：
$$0=Y\frac{\hat{q}S}{K+S}X_aV-bX_aV-QX_a \tag{c}$$
- 展開得：
	$$S=K\frac{1+b(V/Q)}{\hat{q}Y(V/Q)-(1+b(V/Q))}$$
	$$0=-\frac{\hat{q}S}{K+S}X_aV+Q(S^0-S) \tag{d}$$
- 展開得：
	$$X_a=Y\frac{S^0-S}{1+b(V/Q)}$$

定義 $(V/Q)$ 為水力停留時間(hydraulic detention time, HDT) $\theta$，一般情形下，水力停留時間與污泥齡(sludge age, solids retenetion time, SRT)相同：
$${\rm HDT}\ \theta=\frac{V}{Q}{\rm ；SRT}\ \theta_x=\frac{VX_a}{QX_a}{\rm ；}\theta=\theta_x$$

**<font color=green>※</font>** **故得**：
$$S=K\frac{1+b(\theta_x)}{\hat{q}Y(\theta_x)-(1+b(\theta_x))} \tag{1}$$
$$X_a=Y\frac{S^0-S}{1+b(\theta_x)} \tag{2}$$

當水未經處理就流出(出流水基質濃度等於進流水基質濃度($S=S^0$))時，稱之為 Washout，表水力停留時間過短，定義此時之水力停留時間為 $\theta_x^{min}$。故將 $S=S^0$ 帶入式 $(1)$ 可得：
$$\theta_x^{min}=\frac{K+S^0}{S^0(Y\hat{q}-b)-bK} \tag{3}$$

當 $S^0$ 上升時，$\theta_x^{min}$ 亦將上升，最終將達到 $\theta_x^{min}$ 之極限值 $[\theta_x^{min}]_{lim}$，故使用式$(3)$計算 $\theta_x^{min}$ 於 $S^0 \to\infty$ 之極限值，可得：
$$[\theta_x^{min}]_{lim}=\lim_{S^0\to \infty}(\theta_x^{min})=\lim_{S^0\to \infty}(\frac{K+S^0}{S^0(Y\hat{q}-b)-bK})\\=\frac{1}{Y\hat{q}-b}\tag{4}$$

反之，當 $\theta_x$ 上升時，$S$將下降到一極限值 $S_{min}$。故使用式 $(5)$ 計算 $S$ 於 $\theta_x\to\infty$ 之極限值，可得：
$$S_{min}=\lim_{\theta\to\infty}(S)=\lim_{\theta\to\infty}(K\frac{1+b(\theta_x)}{\hat{q}Y(\theta_x)-(1+b(\theta_x))})\\=K\frac{b}{\hat{q}Y-b} \tag{5}$$


## 微生物反應槽惰性菌體及可溶性產物質量平衡

當一微生物反應槽呈穩態時惰性菌體之質量平衡為：
$$\left\{\begin{matrix}
0=(1-f_d)bX_aV+Q(X_i^0-X_i)\\
X_i=X_i^0+X_a(1-f_d)b\theta
\end{matrix}\right.$$
...
而，揮發性懸浮固體物(VSS)之濃度$X_v$為活性菌體濃度($X_a$)及惰性菌體濃度($X_i$)之總和($X_a$帶入上節式(2))，可得：  
$$X_v=X_a+X_i=X_i^0+X_a(1+(1-f_d)b\theta_x)\\=X_i^0+Y(S^0-S)\frac{1+(1-f_d)b\theta_x}{1+b\theta_x}$$
